/* 
 * ßeta.js is a hand-full of help-full stuff for html and javascript
 * In One file of code.
 * 
 * Uses BigEval library
 */
 ...
 var ß = {
    ...
    ra : {
     d : beta_random_dice,
     n : beta_random_number,
     s : beta_random_string
    },
 ...
 function beta_reassign_const(oldvar){
    var value = eval(oldvar);
    eval("delete "+oldvar);
    return eval("const "+oldvar+" = "+value+";");
 }

 function beta_delete(varstr){
    eval("delete "+varstr);
 }

 function beta_reassign_var(oldvar){
    var value = eval(oldvar);
    eval("delete "+oldvar);
    return eval("var "+newvar+" = "+value+";");
 }

 function beta_random_dice(){
    return Math.floor(Math.random() * 7);
 }

 function beta_strip(inp, midsp, ensp){
     var op = inp;
     if(sp){
	for(int g = 0;g<inp.len();g++){
	   if(op[g]==" "){
		op[g] = "";
	   }
	}
 ...
 function beta_removechar(inp, rchar){
     var op = inp;
     for(int g = 0;g<inp.len();g++){
	   if(op[g]==rchar){
		op[g] = "";
 ...
 function beta_random_string(stringlen){
    var outstr = "";
    var chars = "aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPqQrRsStTuUvVwWxXyYzZ!#$^&*?";
    var chrmax = chars.length;
    for(i = 0; i < stringlen; i++){
        outstr += chars[Math.floor(Math.random() * (chrmax))];
    }
    return outstr;
 }

 function beta_random_number(maxnum){
    return Math.floor(Math.random() * (maxnum+1));
 }

 function beta_math(expr){
    var MathExpr = new BigEval();
    return MathExpr.exec(expr);
 }

 function beta_retrieve(dbid, dbname="beta_db"){
    var db_file = localStorage.getItem(dbname);
     var db_parsed = {};
     if(db_file){
         db_parsed = JSON.parse(db_file);
     }else{
         db_parsed = {};
         localStorage.setItem(dbname, JSON.stringify(db_parsed))
     }
     return db_parsed[dbid];
 }
 function beta_store(db, dbid, dbname="beta_db"){
     var db_file = localStorage.getItem(dbname);
     var db_parsed = {};
     if(db_file){
         db_parsed = JSON.parse(db_file);
 ...
 function beta_raw_element(elemname){
    var beta_elem_proto = "";
    if(document.getElementById(elemname)){
        beta_elem_proto = document.getElementById(elemname);
    }else if(document.getElementsByClassName(elemname)){
        beta_elem_proto = document.getElementsByClassName(elemname);
    }else if(document.getElementsByName(elemname)){
        beta_elem_proto = document.getElementsByName(elemname);
    }else if(document.getElementsByTagName(elemname)){
        beta_elem_proto = document.getElementsByTagName(elemname);
    }
    return beta_element_proto;
 }

 function beta_global(varname){
    if(eval(varname)){
        return eval(varname) ;
    }
 }

 function beta_run(coderun){
     return String(eval(coderun));
 }

 function beta_output(stringout, outmode){
    if(outmode==0){
        document.body.innerHTML += stringout;
    }else if(outmode==1){
        console.log(stringout);
    }else if(outmode==2){
        beta_notify(stringout);
    }else if(outmode==3){
        document.body.innerHTML = stringout;
    }
 }

 function beta_notify(textn) {
    if (!("Notification" in window)) {
      return 0;
    }else if (Notification.permission === "granted") {
      var notification = new Notification(textn);
      return 1;
 ...
 function beta_view(elemname, mode, moder){
    if(moder==0){
        document.getElementById(elemname).style.display = (mode == 1) ? "block" : "none";
    }else{
        return ((document.getElementById(elemname).style.display == "block") || document.getElementById(elemname).style.display == "initial") ? 1 : 0;
    }
 }

 function beta_element(elemname){
    var beta_elem_proto = {};
    var method = "";
    if(document.getElementById(elemname)){
        beta_elem_proto = Object(document.getElementById(elemname));
        method = "id";
    }else if(document.getElementsByClassName(elemname)){
        beta_elem_proto = Object(document.getElementsByClassName(elemname));
        method = "clname";
    }else if(document.getElementsByName(elemname)){
        beta_elem_proto = Object(document.getElementsByName(elemname));
        method = "name";
    }else if(document.getElementsByTagName(elemname)){
        beta_elem_proto = Object(document.getElementsByTagName(elemname));
        method = "tagname";
    }
    beta_elem_proto.method = method;
    beta_elem_proto.contents = beta_elem_proto.innerHTML;
    return beta_elem_proto;
 }
...
var DEFAULT_VAR_NAME_CHARS = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_$ß';
